...
"""
    ...
    def analyze_stderr(self, stderr: str) -> Dict[str, Any]:
        ...
        if "typescript" in lowered or "ts" in lowered:
            suggestions.append(
                "运行 `npx tsc -p .` 定位首个 TypeScript 报错并逐个修复。"
            )
        if "cannot find module" in lowered:
            suggestions.append(
                "检查 import 路径与 `tsconfig.json` 的 `paths`/alias 配置，确认依赖已安装。"
            )
        if "module not found" in lowered:
            suggestions.append(
                "确认文件是否存在、大小写是否一致（Linux 容器大小写敏感），以及路径别名是否正确。"
            )
        if "eslint" in lowered:
            suggestions.append(
                "运行 `npm run lint` 查看具体规则与文件位置，按提示修复或调整配置。"
            )
        if "next.config.ts" in lowered:
            suggestions.append(
                "Next.js 14 不支持 `next.config.ts`，请改为 `next.config.js` 或 `next.config.mjs`。"
            )
        if "tailwind" in lowered and ("postcss" in lowered or "unknown" in lowered):
            suggestions.append(
                "检查 `postcss.config.*` 与 `tailwind.config.*` 是否匹配 Tailwind 版本，并确认内容扫描路径覆盖 `app/**/*`。"
            )

        if not suggestions:
            suggestions.append(
                "根据 stderr 首个错误栈定位到文件/行号，优先修复第一个错误再重试构建。"
            )
    ...
    def run(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if report.get("error"):
            ...
            # Set qa_error so Coder can fix it on retry
            error_summary = report.get("error", "build_failed")
            suggestions = report.get("suggestions", [])
            raw = report.get("raw", "")[:500]
            state["qa_error"] = (
                f"{error_summary}: {raw}\nSuggestions: {'; '.join(suggestions)}"
            )
        else:
            state["retry_count"] = retry_count
            state.pop("qa_error", None)
